...
do_start()
{
    ...
    [ -e `dirname "$PIDFILE"` ] || \
        install -d -o{{ cassandra_user }} -g{{ cassandra_group }} -m750 `dirname $PIDFILE`



    start-stop-daemon -S -c {{ cassandra_user }} -a ${CASSANDRA_HOME}/bin/cassandra -q -p "$PIDFILE" -t >/dev/null || return 1

    start-stop-daemon -S -c {{ cassandra_user }} -a ${CASSANDRA_HOME}/bin/cassandra -b -p "$PIDFILE" -- \
